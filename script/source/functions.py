...
def splitData(data, target):
    ...
    sample_submission[target] = np.random.randint(y_train[target].min(), y_train[target].max(), size=(len(test_solution),1))
...
def saveData(path, x_train, test_students, test_solution, sample_submission):
    x_train.to_csv(path + '/train_students.csv', index=False)
